...
public class InvitationController {
    ...
    public ModelAndView showSalaDeEsperaJugadores(Principal principal){
        ...
        String username = principal.getName();
        User jugador = userService.getUserById(username);
        Tablero tablero = invitationService.getTableroActiveUser(jugador);
        if(tablero!=null){
            Partida partida = tablero.getPartida();
            mav.setViewName("redirect:/partida/Multijugador/espera/dado/" + partida.getId());
        }
        mav.addObject("jugadoresAceptados", jugador.getJugadoresAceptados());
        mav.addObject("username", username);
        return mav;
    }
